...
void WaveformHarmonicGenerator::computeWaveformHarmonic(WaveformContainer &h, int l, int m, InspiralContainer &inspiral, double theta, double phi, WaveformHarmonicOptions opts){    
    ...
    rescaleRe = std::real(opts.rescale);
    rescaleIm = std::imag(opts.rescale);
    ...
    #pragma omp parallel num_threads(opts.num_threads)
    {
        ...
        for(int i = 0; i < imax; i++){
            ...
            h.addTimeStep(i, rescaleRe*hplus + rescaleIm*hcross, rescaleRe*hcross - rescaleIm*hplus);
        }
    }
}
...
void WaveformHarmonicGenerator::computeWaveformHarmonicsPhaseAmplitude(WaveformHarmonicsContainer &h, int l[], int m[], double plusY[], double crossY[], int modeNum, InspiralContainer &inspiral, double theta, double phi, WaveformHarmonicOptions opts){
    ...
    rescaleAmp = std::abs(opts.rescale);
    rescalePhase = std::arg(opts.rescale);
    ...
    #pragma omp parallel num_threads(opts.num_threads)
    {
      ...
      for(j = 0; j < modeNum; j++){
        for(i = 0; i < imax; i++){
          ...
          // positive m-modes
          h.setTimeStep(2*j, i, rescaleAmp*amp*plusY[j], rescalePhase + Phi);
          // negative m-modes
          h.setTimeStep(2*j + 1, i, rescaleAmp*amp*crossY[j], - Phi - l[j]*M_PI + rescalePhase);
        }
      }
    }
}
...
void WaveformGenerator::computeWaveformSourceFrame(WaveformContainer &h, double M, double mu, double a, double r0, double theta, double phi, double Phi_phi0, double dt, double T, WaveformHarmonicOptions opts){
	dt = convertTime(dt, M);
	T = convertTime(years_to_seconds(T), M);
...
void WaveformGenerator::computeWaveformSourceFrame(WaveformContainer &h, int l[], int m[], int modeNum, double M, double mu, double a, double r0, double theta, double phi, double Phi_phi0, double dt, double T, WaveformHarmonicOptions opts){
	dt = convertTime(dt, M);
	T = convertTime(years_to_seconds(T), M);
	// WaveformHarmonicOptions opts = getWaveformHarmonicOptions();
...
void WaveformGenerator::computeWaveformSourceFrame(WaveformHarmonicsContainer &h, int l[], int m[], int modeNum, double M, double mu, double a, double r0, double theta, double phi, double Phi_phi0, double dt, double T, WaveformHarmonicOptions opts){
	dt = convertTime(dt, M);
	T = convertTime(years_to_seconds(T), M);
	// WaveformHarmonicOptions opts = getWaveformHarmonicOptions();
