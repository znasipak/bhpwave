...
EigenCubicInterpolator read_time_spline(double){
  TrajectoryData traj = read_trajectory_data();
  return EigenCubicInterpolator(traj.alpha, traj.t);
}

EigenCubicInterpolator read_phase_spline(double){
  TrajectoryData traj = read_trajectory_data();
  return EigenCubicInterpolator(traj.alpha, traj.phi);
}

EigenCubicInterpolator read_flux_spline(double){
  TrajectoryData traj = read_trajectory_data();
  return EigenCubicInterpolator(traj.alpha, traj.flux);
}
...
TrajectorySpline::TrajectorySpline(const double & chi, const EigenArray & alpha, const EigenArray & t, const EigenArray & phi, const EigenArray & flux):
...
EigenCubicInterpolator TrajectorySpline::get_phase_spline(){
	EigenCubicInterpolator phase_copy(_phase_spline);
	return phase_copy;
}
...
SmallTrajectorySpline2D::SmallTrajectorySpline2D(const EigenArray &chi, const EigenArray & alpha, const EigenArray &flux): _flux_spline(chi, alpha, flux) {}
...
double TrajectorySpline2D::orbital_alpha(double chi, double t){
  // std::cout << "alpha t = " << t << "\n";
  // std::cout << "alpha ratio = " << sqrt(log(1. - t))/_max_time_spline.evaluate(chi) << "\n";
...
// EigenBicubicInterpolator TrajectorySpline2D::get_phase_spline(){
//   EigenBicubicInterpolator phase_copy(_phase_spline);
//   return phase_copy;
// }
